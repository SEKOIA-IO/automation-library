...
class UbikaCloudProtectorNextGenTrafficLogsConnector(Connector):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.client = UbikaCloudProtectorNextGenApiClient(
    ...
    def _handle_response_error(self, response: httpx.Response) -> None:
        ...
        if not response.is_success:
            try:
                ...
                err = response.text
            msg = (
                f"[{self.NAME}] HTTP {response.status_code} error: "
                f"{err} for {response.request.url}"
            )
    ...
    def _fetch_pages(self, start_timestamp: int) -> Generator[list[dict], None, None]:
        ...
        url = (
            f"https://api.ubika.io/rest/logs.ubika.io/v1"
            f"/ns/{self.configuration.namespace}/traffic-logs"
        )
