...
def attribute_data_pattern(data):
    return f"artifact:payload_bin = '{data}'"
...
def pattern_as(_, attribute_value):
    return f"[autonomous-system:number = '{attribute_value}']"
...
def pattern_attachment(*args):
    ...
    if len(args) == 3:
        pattern += f" AND {attribute_data_pattern(args[2])}"
    return f"[{pattern}]"
...
def pattern_domain_ip(_, attribute_value):
    ...
    domain += f" AND domain-name:resolves_to_refs[*].value = '{ip_value}'"
    return f"[{domain}]"
...
def observable_email_address(attribute_type, attribute_value):
    ...
    return {
        "0": {"type": "email-addr", "value": attribute_value},
...
def pattern_file_hash(attribute_type, attribute_value):
    _, hash_type = attribute_type.split("|")
    value1, value2 = attribute_value.split("|")
    return f"[file:name = '{value1}' AND file:hashes.'{hash_type}' = '{value2}']"
...
def pattern_hostname_port(_, attribute_value):
    hostname, port = attribute_value.split("|")
    return f"[{pattern_domain(_, hostname)[1:-1]} AND {pattern_port(_, port)[1:-1]}]"
...
def observable_ip(attribute_type, attribute_value):
    ...
    address_type = define_address_type(attribute_value)
    return {
        "0": {"type": address_type, "value": attribute_value},
        "1": {
...
def pattern_ip(attribute_type, attribute_value):
    ip_type = attribute_type.split("-")[1]
    address_type = define_address_type(attribute_value)
    return "[network-traffic:{0}_ref.type = '{1}' AND network-traffic:{0}_ref.value = '{2}']".format(
        ip_type, address_type, attribute_value
    )
...
def pattern_ip_port(attribute_type, attribute_value):
    ...
    ip, port = attribute_value.split("|")
    port_type = "{}_port".format(ip_type.split("-")[1])
    return f"[network-traffic:{port_type} = '{port}' AND {pattern_ip(ip_type, ip)[1:-1]}]"
...
def pattern_mac_address(_, attribute_value):
    return f"[mac-addr:value = '{attribute_value}']"
...
def pattern_regkey_value(_, attribute_value):
    ...
    regkey += f" AND windows-registry-key:values = '{value.strip()}'"
    return f"[{regkey}]"
...
def observable_reply_to(_, attribute_value):
    return {
        ...
        "1": {
            "type": "email-message",
            "additional_header_fields": {"Reply-To": ["0"]},
            "is_multipart": "false",
        },
    }
...
def pattern_x509(_, attribute_value):
    return f"[x509-certificate:hashes = '{attribute_value}']"
...
mispTypesMapping = {
    ...
    "vulnerability": {
        "to_call": "add_vulnerability",
        "vulnerability_args": return_vulnerability,
    },
    "md5": {
        "to_call": "handle_usual_type",
    ...
    "filename|md5": {
        "to_call": "handle_usual_type",
        "observable": observable_file_hash,
        "pattern": pattern_file_hash,
    },
    "filename|sha1": {
        "to_call": "handle_usual_type",
        "observable": observable_file_hash,
    ...
    "email-src": {
        "to_call": "handle_usual_type",
        "observable": observable_email_address,
        "pattern": pattern_email_address,
    },
    "email-dst": {
        "to_call": "handle_usual_type",
        "observable": observable_email_address,
    ...
    "email-subject": {
        "to_call": "handle_usual_type",
        "observable": observable_email_message,
        "pattern": pattern_email_message,
    },
    "email-body": {
        "to_call": "handle_usual_type",
        "observable": observable_email_message,
    ...
    "email-attachment": {
        "to_call": "handle_usual_type",
        "observable": observable_email_attachment,
        "pattern": pattern_email_attachment,
    },
    "url": {
        "to_call": "handle_usual_type",
    ...
    "regkey|value": {
        "to_call": "handle_usual_type",
        "observable": observable_regkey_value,
        "pattern": pattern_regkey_value,
    },
    "malware-sample": {
        "to_call": "handle_usual_type",
        "observable": observable_malware_sample,
        "pattern": pattern_malware_sample,
    },
    "mutex": {
        "to_call": "handle_usual_type",
        "observable": observable_mutex,
        "pattern": pattern_mutex,
    },
    "uri": {
        "to_call": "handle_usual_type",
        "observable": observable_url,
    ...
    "hostname|port": {
        "to_call": "handle_usual_type",
        "observable": observable_hostname_port,
        "pattern": pattern_hostname_port,
    },
    "email-reply-to": {
        "to_call": "handle_usual_type",
        "observable": observable_reply_to,
        "pattern": pattern_reply_to,
    },
    "attachment": {
        "to_call": "handle_usual_type",
        "observable": observable_attachment,
        "pattern": pattern_attachment,
    },
    "mac-address": {
        "to_call": "handle_usual_type",
        "observable": observable_mac_address,
        "pattern": pattern_mac_address,
    },
    "AS": {
        "to_call": "handle_usual_type",
        "observable": observable_as,
        "pattern": pattern_as,
    },
}
...
objectsMapping = {
    "asn": {
        "to_call": "handle_usual_object_name",
...
emailObjectMapping = {
    "email-body": {"email_type": "message", "stix_type": "body"},
    ...
    "reply-to": {
        "email_type": "message",
        "stix_type": "additional_header_fields.reply_to",
    },
    "attachment": {
        "email_type": "message",
        "stix_type": "body_multipart[*].body_raw_ref.name",
    },
    "send-date": {"email_type": "message", "stix_type": "date"},
    "x-mailer": {
        "email_type": "message",
        "stix_type": "additional_header_fields.x_mailer",
    },
}
...
networkSocketMapping = {
    ...
    "src-port": "src_port",
    "dst-port": "dst_port",
    "ip-src": network_traffic_src_ref,
    "ip-dst": network_traffic_dst_ref,
    "hostname-src": network_traffic_src_ref,
    "hostname-dst": network_traffic_dst_ref,
}
