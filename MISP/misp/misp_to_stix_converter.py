...
import pymisp
from misp.misp2stix2_mapping import (
    asnObjectMapping,
    attribute_data_pattern,
    define_address_type,
    defineProtocols,
    domainIpObjectMapping,
    emailObjectMapping,
    fileMapping,
    ipPortObjectMapping,
    mispTypesMapping,
    networkSocketMapping,
    objectsMapping,
    peMapping,
    peSectionMapping,
    processMapping,
    regkeyMapping,
    relationshipsSpecifications,
    urlMapping,
    x509mapping,
)
...
class STIXConverter:
    ...
    def convert(self, event):
        ...
        objects = self.handler(event["Event"])
        bundle = Bundle(objects=objects)
        # Will convert all the required fields to a primitive type
    ...
    def __set_identity(self):
        ...
        org_uuid = org["uuid"]
        identity_id = f"identity--{org_uuid}"
    ...
    def load_objects_mapping(self):
        self.objects_mapping = {
            "asn": {
                "observable": self.resolve_asn_observable,
                "pattern": self.resolve_asn_pattern,
            },
            "domain-ip": {
                "observable": self.resolve_domain_ip_observable,
                "pattern": self.resolve_domain_ip_pattern,
            },
            "email": {
                "observable": self.resolve_email_object_observable,
                "pattern": self.resolve_email_object_pattern,
            },
            "file": {
                "observable": self.resolve_file_observable,
                "pattern": self.resolve_file_pattern,
            },
            "ip-port": {
                "observable": self.resolve_ip_port_observable,
                "pattern": self.resolve_ip_port_pattern,
            },
            "network-socket": {
                "observable": self.resolve_network_socket_observable,
                "pattern": self.resolve_network_socket_pattern,
            },
            "process": {
                "observable": self.resolve_process_observable,
                "pattern": self.resolve_process_pattern,
            },
            "registry-key": {
                "observable": self.resolve_regkey_observable,
                "pattern": self.resolve_regkey_pattern,
            },
            "stix2": {"pattern": self.resolve_stix2_pattern},
            "url": {
                "observable": self.resolve_url_observable,
                "pattern": self.resolve_url_pattern,
            },
            "x509": {
                "observable": self.resolve_x509_observable,
                "pattern": self.resolve_x509_pattern,
            },
        }
        self.galaxies_mapping = {
            "branded-vulnerability": [
                "vulnerability",
                self.add_vulnerability_from_galaxy,
            ]
        }
        self.galaxies_mapping.update(
            dict.fromkeys(
                attack_pattern_galaxies_list,
                ["attack-pattern", self.add_attack_pattern],
            )
        )
        self.galaxies_mapping.update(
            dict.fromkeys(
                course_of_action_galaxies_list,
                ["course-of-action", self.add_course_of_action],
            )
        )
        self.galaxies_mapping.update(
            dict.fromkeys(intrusion_set_galaxies_list, ["intrusion-set", self.add_intrusion_set])
        )
        self.galaxies_mapping.update(dict.fromkeys(malware_galaxies_list, ["malware", self.add_malware]))
        self.galaxies_mapping.update(
            dict.fromkeys(threat_actor_galaxies_list, ["threat-actor", self.add_threat_actor])
        )
    ...
    def get_object_by_uuid(self, uuid):
        for _object in self.misp_event["Object"]:
            if _object.get("uuid") and _object["uuid"] == uuid:
                return _object
        raise Exception(f"Object with uuid {uuid} does not exist in this event.")
        ...
        if name == "file" and misp_object.get("ObjectReference"):
            for reference in misp_object["ObjectReference"]:
                if reference["relationship_type"] == "included-in" and reference["Object"]["name"] == "pe":
                    self.objects_to_parse[name][misp_object["uuid"]] = (
                        to_ids,
                        misp_object,
                    )
                    return
    ...
    def populate_objects_to_parse(self, misp_object, to_ids):
        self.objects_to_parse[misp_object["name"]][misp_object["uuid"]] = (
            to_ids,
            misp_object,
        )

    def resolve_objects2parse(self):
        for _, misp_object in self.objects_to_parse["file"].items():
            ...
            for reference in pe_object["ObjectReference"]:
                if (
                    reference["Object"]["name"] == "pe-section"
                    and reference["referenced_uuid"] in self.objects_to_parse["pe-section"]
                ):
            ...
            if True in to_ids_list:
                ...
                pattern += f" AND {self.parse_pe_extensions_pattern(pe_object, sections)}"
                self.add_object_indicator(file_object, pattern_arg=f"[{pattern}]")
            else:
    ...
    def _create_pe_type_test(observable, extension):
        return [
            ...
            (
                "mime_type" in observable
                and re.compile(".* .+{0}.+ .*|.* {0} .*".format(extension)).match(observable["mime_type"].lower())
            ),
        ]
    ...
    def parse_pe_extensions_observable(self, pe_object, sections, pe_type):
        ...
        for attribute in pe_object["Attribute"]:
            try:
                ...
                extension[
                    "x_misp_{}_{}".format(
                        attribute["type"],
                        attribute["object_relation"].replace("-", "_"),
                    )
                ] = attribute["value"]
        for section in sections:
            ...
            if "name" not in d_section:
                d_section["name"] = f"Section {sections.index(section)}"
    ...
    def parse_pe_extensions_pattern(self, pe_object, sections):
        ...
        for section in sections:
            ...
            for attribute in section["Attribute"]:
                relation = attribute["object_relation"]
                if relation in misp_hash_types:
                    stix_type = f"{section_mapping}.hashes.'{relation}'"
                    pattern += mapping.format(stix_type, attribute["value"])
                else:
                    try:
                        stix_type = f"{section_mapping}.{peSectionMapping[relation]}"
                        pattern += mapping.format(stix_type, attribute["value"])
                    except KeyError:
                        continue
            n_section += 1
    ...
    def parse_galaxies(self, galaxies, source_id=None):
        for galaxy in galaxies:
            self.parse_galaxy(galaxy, source_id)

    def parse_galaxy(self, galaxy, source_id=None):
    ...
    def generate_galaxy_args(galaxy, b_killchain, b_alias, sdo_type):
        ...
        sdo_id = f"{sdo_type}--{cluster_uuid}"
        ...
        if b_killchain:
            killchain = [{"kill_chain_name": "misp-category", "phase_name": galaxy["type"]}]
    ...
    def add_indicator(self, attribute):
        ...
        attribute_value = (
            attribute["value"]
            if attribute_type != "AS"
            else self.define_attribute_value(attribute["value"], attribute["comment"])
        )
        pattern = (
            mispTypesMapping[attribute_type]["pattern"](attribute_type, attribute_value, attribute["data"])
            if attribute.get("data")
            else self.define_pattern(attribute_type, attribute_value)
        )
        indicator_args = {
            "id": indicator_id,
            "type": "indicator",
    ...
    def add_observed_data(self, attribute):
        ...
        observable = (
            mispTypesMapping[attribute_type]["observable"](attribute_type, attribute_value, attribute["data"])
            if attribute.get("data")
            else self.define_observable(attribute_type, attribute_value)
        )
        observed_data_args = {
            "id": observed_data_id,
            "type": "observed-data",
    ...
    def add_vulnerability(self, attribute):
        ...
        vulnerability_args = {
            ...
            "type": "vulnerability",
            "name": name,
            "external_references": vulnerability_data,
            "created_by_ref": self.identity_id,
            "labels": labels,
        }
        if markings:
    ...
    def add_vulnerability_from_galaxy(self, attribute):
        ...
        if cluster["meta"] and cluster["meta"]["aliases"]:
            vulnerability_data = [
                mispTypesMapping["vulnerability"]["vulnerability_args"](alias) for alias in cluster["meta"]["aliases"]
            ]
        else:
            vulnerability_data = [mispTypesMapping["vulnerability"]["vulnerability_args"](name)]
        ...
        if name == "network-socket":
            ...
            for field in ("address_family", "protocol_family"):
                ...
                try:
                    ...
                    exceptions.MissingPropertiesError,
                ):
    ...
    def create_labels(attribute):
        labels = [
            'misp:type="{}"'.format(attribute["type"]),
    ...
    def create_marking(self, tag):
        id_ = "marking-definition--%s" % uuid.uuid4()
    ...
    def define_observable(attribute_type, attribute_value):
        ...
        if attribute_type == "port":
            observable["0"]["protocols"].append(
                defineProtocols[attribute_value] if attribute_value in defineProtocols else "tcp"
            )
        return observable
    ...
    def define_pattern(attribute_type, attribute_value):
        attribute_value = (
            attribute_value.replace("'", "##APOSTROPHE##").replace('"', "##QUOTE##")
    ...
    def resolve_asn_observable(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            if relation == "subnet-announced":
                observable[str(object_num)] = {
                    "type": define_address_type(attribute_value),
    ...
    def resolve_asn_pattern(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            attribute_value = attribute["value"]
            if relation == "subnet-announced":
                pattern += f"{define_address_type(attribute_value)}:{stix_type} = '{attribute_value}' AND "
            else:
    ...
    def resolve_domain_ip_observable(self, attributes, object_id):
        ...
        domain_ip_value = f"{domain_value}|{ip_value}"
        ...
        for attribute in attributes:
            ...
            try:
                ...
                if relation in ("from", "to", "cc"):
                    ...
                    object_str = str(object_num)
                    body = {
                        "content_disposition": f"{relation}; filename='{attribute_value}'",
    ...
    def resolve_email_object_pattern(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            try:
                ...
                stix_type = "'x_misp_{}_{}'".format(attribute["type"], relation)
                if relation in ("eml", "screenshot"):
                    stix_type_data = f"{stix_type}.data"
    ...
    def resolve_file_observable(self, attributes, object_id):
        ...
        if "md5" in d_observable:
            observable_file["hashes"]["MD5"] = (
                malware_sample["md5"] if "md5" in malware_sample else d_observable["md5"]
            )
        if "filename" in d_observable:
            observable_file["name"] = (
                malware_sample["filename"] if "filename" in malware_sample else d_observable["filename"]
            )
    ...
    def resolve_file_pattern(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            if attribute_type == "malware-sample":
                ...
                try:
                    stix_type = (
                        fileMapping["hashes"].format(attribute_type)
                        if attribute_type in misp_hash_types
                        else fileMapping[attribute_type]
                    )
                except KeyError:
    ...
    def resolve_network_socket_observable(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            if relation in ("address-family", "domain-family"):
                ...
                ip_src = attribute["value"]
            elif relation == "ip-dst":
                ip_dst = attribute["value"]
            elif relation == "hostname-src":
                domain_src = attribute["value"]
            elif relation == "hostname-dst":
                domain_dst = attribute["value"]
            else:
                try:
    ...
    def resolve_network_socket_pattern(self, attributes, object_id):
        ...
        for attribute in attributes:
            ...
            if relation in ("address-family", "domain-family"):
                ...
                state_type = f"is_{attribute_value}"
                pattern += mapping.format(stix_type.format(state_type), True)
            elif relation == "protocol":
                pattern += f"network-traffic:{networkSocketMapping[relation]}[0] = '{attribute_value}' AND "
            elif relation == "ip-src":
                ip_src = mapping.format(
                    networkSocketMapping[relation].format(define_address_type(attribute_value)),
                    attribute_value,
                )
            elif relation == "ip-dst":
                ip_dst = mapping.format(
                    networkSocketMapping[relation].format(define_address_type(attribute_value)),
                    attribute_value,
                )
            elif relation == "hostname-src":
                domain_src = mapping.format(
                    networkSocketMapping[relation].format("domain-name"),
                    attribute_value,
                )
            elif relation == "hostname-dst":
                domain_dst = mapping.format(
                    networkSocketMapping[relation].format("domain-name"),
                    attribute_value,
                )
            else:
