...
def trigger(symphony_storage):
    ...
    trigger = M365EventsTrigger(module=module, data_path=symphony_storage)
...
def test_fetch_events_should_chunk_events(trigger):
    url_emails = (
        f"{trigger.module.configuration.api_host}/api/v1/tenants/"
        f"{trigger.configuration.tenant_id}/logs/emails/search"
    )
    url_auto_remediation = (
        f"{trigger.module.configuration.api_host}/api/v1/tenants/"
        f"{trigger.configuration.tenant_id}/logs/remediations/auto/search"
    )
    url_manual_remediation = (
        f"{trigger.module.configuration.api_host}/api/v1/tenants/"
    ...
    with requests_mock.Mocker() as mock, patch("vadesecure_modules.trigger_m365_events.time.sleep"):
        ...
        mock.post(url_emails, json={"result": {"total": 2, "messages": [message1, message2]}})
        ...
        mock.post(
            url_auto_remediation,
            json={"result": {"total": 2, "campaigns": [campaign1]}},
        )
        mock.post(url_manual_remediation, json={"result": {"total": 0, "campaigns": []}})

        trigger._fetch_events()
        assert trigger.send_event.call_count == 3
        call_args = trigger.send_event.call_args.kwargs
        assert "emails_path" in call_args["event"].keys()
