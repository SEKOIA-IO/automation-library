...
class ThreatVisualizerLogConsumer(Thread):
    ...
    def request_events(self) -> requests.models.Response:
        ...
        url = urljoin(self.connector.module.configuration.api_url,
                      self.endpoint.value)
    ...
    def next_batch(self):
        ...
        try:
            ...
            INCOMING_MESSAGES.labels(
                intake_key=self.connector.configuration.intake_key).inc(len(response))
        ...
        if type(response) is list:
            ...
            if response != []:
                ...
                batch_of_events = [orjson.dumps(event).decode(
                    "utf-8") for event in response]

                OUTCOMING_EVENTS.labels(
                    intake_key=self.connector.configuration.intake_key).inc(len(batch_of_events))
                ...
                EVENTS_LAG.labels(intake_key=self.connector.configuration.intake_key).observe(
                    int(current_lag))
        ...
        FORWARD_EVENTS_DURATION.labels(
            intake_key=self.connector.configuration.intake_key).observe(batch_duration)
        ...
        while self.running:
            try:
                ...
                self.connector.log_exception(
                    error, message="Failed to forward events")
...
class ThreatVisualizerLogConnector(Connector):
    ...
    def start_consumers(self):
        ...
        for endpoint in Endpoints:
            self.log(message=f"Start {endpoint.name} consumer",
    ...
    def supervise_consumers(self, consumers):
        ...
        for endpoint, consumer in consumers.items():
            if consumer is None or (not consumer.is_alive() and consumer.running):
                self.log(
                    message=f"Restarting {endpoint.name} consumer", level="info")  # pragma: no cover
    ...
    def stop_consumers(self, consumers):
        for endpoint, consumer in consumers.items():
            if consumer is not None and consumer.is_alive():
                self.log(
                    message=f"Stopping {endpoint.name} consumer", level="info")  # pragma: no cover
                consumer.stop()
