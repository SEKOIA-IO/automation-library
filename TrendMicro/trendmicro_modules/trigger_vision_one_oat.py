...
class TrendMicroVisionOneOATConnectorConfiguration(DefaultConnectorConfiguration):
...
class TrendMicroVisionOneOATConnector(Connector):
    ...
    def __fetch_events(self, from_date: datetime) -> Generator[list, None, None]:
        ...
        while self.running:
            ...
            print(message)
            if events:
                INCOMING_MESSAGES.labels(intake_key=self.configuration.intake_key, type="vision_one_oat").inc(
                    len(events)
                )
                yield events

            else:
                EVENTS_LAG.labels(intake_key=self.configuration.intake_key, type="vision_one_oat").set(0)
                return
    ...
    def fetch_events(self) -> Generator[list, None, None]:
        ...
        EVENTS_LAG.labels(intake_key=self.configuration.intake_key, type="vision_one_oat").set(
    ...
    def next_batch(self):
        ...
        for events in self.fetch_events():
            ...
            if len(batch_of_events) > 0:
                ...
                OUTCOMING_EVENTS.labels(intake_key=self.configuration.intake_key, type="vision_one_oat").inc(
                    len(batch_of_events)
                )
        ...
        FORWARD_EVENTS_DURATION.labels(intake_key=self.configuration.intake_key, type="vision_one_oat").observe(
            batch_duration
        )
    ...
    def run(self) -> None:
        self.log(message="Start fetching Trend Micro Vision One OAT logs", level="info")
