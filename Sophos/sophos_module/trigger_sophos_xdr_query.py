...
from sophos_module.metrics import (
    EVENTS_LAG,
...
class SophosXDRQueryTrigger(SophosConnector):
    ...
    def post_query(self, query: dict[str, Any]) -> tuple[str, str | None]:
        ...
        # Posting the query
        self.log(
            message=f"Querying the Sophos data lake at {self.from_date.strftime('%Y-%m-%d %H:%M:%S')}",
            level="info",
        )
        ...
        self.log(
            message=f"Finishing the loop with status {status} and result {result}",
            level="info",
        )
