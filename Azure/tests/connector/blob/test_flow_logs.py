...
def connector(symphony_storage, container_name, account_name, account_key, session_faker, mock_push_data_to_intakes):
    ...
    trigger = AzureFlowLogsConnector(
        module=module,
...
@pytest.mark.asyncio
async def test_flow_logs_last_event_date(connector):
    """
...
@pytest.mark.asyncio
async def test_flow_logs_get_network_watcher_data_1(
    connector: AzureNetworkWatcherConnector, session_faker, flow_logs_content
):
    """
    ...
    download_blob_result.return_value = (None, flow_logs_content)
    ...
    assert result == connector.filter_blob_data(flow_logs_content.decode("utf-8"))


@pytest.mark.asyncio
async def test_flow_logs_get_network_watcher_data_2(
    connector: AzureNetworkWatcherConnector, session_faker, symphony_storage, flow_logs_content
):
    """
    Test AzureNetworkWatcherConnector get events.
    ...
    async with aiofiles.tempfile.NamedTemporaryFile("wb", delete=False, dir="") as file:
        ...
        await file.write(flow_logs_content)
...
@pytest.mark.asyncio
async def test_flow_logs_get_network_watcher_data_3(
    connector: AzureNetworkWatcherConnector, session_faker, symphony_storage, flow_logs_content
):
    """
    Test AzureNetworkWatcherConnector get events.
