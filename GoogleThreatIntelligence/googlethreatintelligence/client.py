...
class VTAPIConnector:
    ...
    def __init__(self, api_key: str, domain: Optional[str] = None, 
                 ip: Optional[str] = None, url: Optional[str] = None, 
                 file_hash: Optional[str] = None, cve: Optional[str] = None):
        """
        ...
        try:
            ...
            for comment in comments_it:
                comment_data = {
                    ...
                    } if hasattr(comment, 'votes') else {"positive": 0, "negative": 0},
    ...
    def get_passive_dns(self, client: vt.Client):
        ...
        try:
            ...
            logger.info(f"Retrieved and processed {len(resolutions)} DNS resolutions with {len(unique_ips)} unique IPs")
    ...
    def get_vulnerability_report(self, client: vt.Client):
        ...
        try:
            ...
            if hasattr(vuln, 'description'):
                vuln_data["description"] = vuln.description[:200] + "..." if len(vuln.description) > 200 else vuln.description
    ...
    def get_vulnerability_associations(self, client: vt.Client):
        ...
        try:
            ...
            for vuln in vulns_it:
                ...
                if hasattr(vuln, 'description'):
                    vuln_data["description"] = vuln.description[:150] + "..." if len(vuln.description) > 150 else vuln.description
            ...
            logger.info(f"Retrieved and processed {len(vulnerabilities)} vulnerability associations ({len(cve_ids)} unique CVEs)")
