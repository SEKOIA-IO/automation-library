...
@patch("googlethreatintelligence.actions.get_passive_dns.vt.Client")
...
@patch("googlethreatintelligence.actions.get_passive_dns.vt.Client")
def test_get_passive_dns_generic_exception(mock_vt_client):
    """Test generic exception handling."""
    mock_vt_client.side_effect = RuntimeError("connection lost")
    ...
    assert isinstance(response, dict)
    assert response.get("success") is False
    assert "connection lost" in response.get("error", "")
