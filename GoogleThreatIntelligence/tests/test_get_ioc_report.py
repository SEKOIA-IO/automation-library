...
@patch("googlethreatintelligence.actions.get_ioc_report.vt.Client")
@patch("googlethreatintelligence.actions.get_ioc_report.VTAPIConnector")
def test_get_domain_report_success(mock_connector_class, mock_vt_client):
    ...
    mock_connector_class.assert_called_once_with(API_KEY, domain=DOMAIN, ip="", url="", file_hash="")
...
def test_get_ip_report_success(mock_connector_class, mock_vt_client):
    ...
    mock_result = MagicMock()
    mock_result.status = "SUCCESS"
    mock_result.response = {"entity_type": "ip_addresses", "entity": IP, "id": IP, "reputation": 95, "country": "US"}
    mock_result.error = None
    mock_connector.results = [mock_result]
    ...
    # Verify VTAPIConnector was called correctly
    mock_connector_class.assert_called_once_with(API_KEY, domain="", ip=IP, url="", file_hash="")
...
def test_get_url_report_success(mock_connector_class, mock_vt_client):
    ...
    # Verify VTAPIConnector was called correctly
    mock_connector_class.assert_called_once_with(API_KEY, domain="", ip="", url=URL, file_hash="")
...
def test_get_file_report_success(mock_connector_class, mock_vt_client):
    ...
    # Verify VTAPIConnector was called correctly
    mock_connector_class.assert_called_once_with(API_KEY, domain="", ip="", url="", file_hash=FILE_HASH)
