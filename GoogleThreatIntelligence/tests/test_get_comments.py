...
@patch("googlethreatintelligence.actions.get_comments.vt.Client")
...
# =============================================================================
# Default entity (no entity provided → fallback to domains)
# =============================================================================


@patch("googlethreatintelligence.actions.get_comments.vt.Client")
def test_get_comments_default_entity(mock_vt_client):
    """Test that default entity is used when all fields are empty."""
    mock_client = MagicMock()
    mock_vt_client.return_value.__enter__.return_value = mock_client
    mock_client.iterator.return_value = iter([mock_comment()])
    ...
    # All fields empty → should fallback to default entity (domains)
    response = action.run({"domain": "", "ip": "", "url": "", "file_hash": ""})

    assert isinstance(response, dict)
    assert response.get("success") is True

    # Should have used domains as entity_type
    args, _ = mock_client.iterator.call_args
    assert args[0].startswith("/domains/")
...
@patch("googlethreatintelligence.actions.get_comments.vt.Client")
def test_get_comments_generic_exception(mock_vt_client):
    """Test generic exception is caught and returns error."""
    mock_vt_client.side_effect = RuntimeError("unexpected error")
    ...
    assert isinstance(response, dict)
    assert response.get("success") is False
    assert "unexpected error" in response.get("error", "")
