...
class BitsightModuleConfiguration(BaseModel):
    ...
    api_token: str = Field(description="The API token to authenticate call to the Bitsight API")

    company_uuids: list[str] = Field(description="The company uuids to fetch data from Bitsight API")
