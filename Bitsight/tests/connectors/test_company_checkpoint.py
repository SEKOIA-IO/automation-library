...
def test_with_updated_last_seen_within_1_days_1():
    ...
    """
    recent_date = (datetime.now(UTC) - timedelta(minutes=10)).strftime("%Y-%m-%d")
...
def test_with_updated_last_seen_within_1_days_2():
    ...
    Test that the last_seen date is not updated if it is within 1 day.
    """
    recent_date = (datetime.now(UTC).replace(microsecond=0, second=0, minute=0, hour=0) - timedelta(days=1)).strftime(
        "%Y-%m-%d"
    )
