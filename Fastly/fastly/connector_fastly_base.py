...
class FastlyBasicConnectorConfiguration(DefaultConnectorConfiguration):
...
class FastlyConnectorBase(Connector):
    base_uri = "https://dashboard.signalsciences.net"
    configuration: FastlyBasicConnectorConfiguration
    ...
    def fetch_events(self) -> Generator[list, None, None]:
        ...
        try:
            for next_events in self.fetch_next_events(most_recent_date_seen):
                if next_events:
                    last_event_datetime = self.get_datetime_from_item(next_events[-1])
