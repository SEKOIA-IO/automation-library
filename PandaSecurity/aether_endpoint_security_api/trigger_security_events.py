...
class AetherSecurityEventsTrigger(Trigger, AuthorizationMixin):
    ...
    def _fetch_events(self) -> None:
        ...
        for event_type, event_type_name in EVENT_TYPES.items():
            ...
            if message_batch:
                INCOMING_MESSAGES.labels(type=event_type_name, **self.scalability_labels).inc(len(message_batch))
                ...
                OUTCOMING_EVENTS.labels(type=event_type_name, **self.scalability_labels).inc(len(message_batch))

            # get the ending time and compute the duration to fetch the events
            batch_end_time = time.time()
            batch_duration = int(batch_end_time - batch_start_time)
            FORWARD_EVENTS_DURATION.labels(type=event_type_name, **self.scalability_labels).observe(batch_duration)
            ...
            EVENTS_LAG.labels(type=event_type_name, **self.scalability_labels).set(events_lag)
