...
from .helpers import parse_team
from .metrics import EVENTS_LAG


class BeyondTrustPRATeamConfiguration(DefaultConnectorConfiguration):
    frequency: int = Field(5 * 60, description="Batch frequency in seconds")


class BeyondTrustPRATeamConnector(BeyondTrustBaseConnector):
    module: BeyondTrustModule
    configuration: BeyondTrustPRATeamConfiguration
    ...
    def fetch_events(self) -> Generator[list, None, None]:
        ...
        response = self.client.get_team(end_time=most_recent_date_seen)
        ...
        events = parse_team(response.content)
        if len(events) > 0:
            yield events
