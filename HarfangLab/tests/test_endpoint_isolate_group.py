...
from harfanglab.endpoint_actions import EndpointGroupIsolationAction
...
def test_integration_isolate_group(symphony_storage):
    ...
    action = EndpointGroupIsolationAction(data_path=symphony_storage)
    ...
    assert len(response.get("requested", [])) > 0
