...
from ..models import GetBlockListActionRequest, GetBlockListActionResponse, ItemsModel, RuleModel, HashModel, ConnectionModel, RemoteAddressModel, LocalAddressModel, BlockListModel, RemoveBlockActionRequest, DirectlyConnectedModel
from ..bitdefender_gravity_zone_api import prepare_push_block_endpoint, prepare_remove_block_endpoint, prepare_get_block_list_endpoint
...
class PushIocsAction(BitdefenderAction):
    ...
    def indicator_to_rule(self, type: str, value: str, path: list[str]) -> RuleModel:
        ...
        match type:
            ...
            case "ipv4-addr":
                ...
                details = ConnectionModel(rule_name=f"{value}",
                                          ip_version="IPV4",
                                          protocol="any",
                                          direction="both",
    ...
    def get_existing_rules(self) -> list[ItemsModel]:
        ...
        block_list_response: GetBlockListActionResponse = parse_get_block_list_response(self.execute_request(
        ...
        while page <= nb_pages:
            ...
            block_list_response = parse_get_block_list_response(self.execute_request(
            prepare_get_block_list_endpoint(block_list_request.dict(exclude_none=True, by_alias=True))))
    ...
    def get_valid_rules(self, stix_objects) -> Dict[str, list]:
        ...
        for object in stix_objects:
            ...
            self.log(
                message=f"object in stix_objects {str(object)}", level="debug")
    ...
    def add_rules(self, rules: list[RuleModel]):
        ...
        if len(add_hashes) > 0:
            ...
            self.execute_request(prepare_push_block_endpoint(
                args.dict(exclude_none=True, by_alias=True)))
    ...
    def run(self, arguments: dict) -> dict:
        ...
        fetch_rules = self.get_valid_rules(
            stix_objects)
