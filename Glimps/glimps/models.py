...
class GLIMPSConfiguration(BaseModel):
    api_key: str = Field(..., json_schema_extra={"secret": True}, description="Glimps detect token")
...
class SubmitArgument(BaseModel):
    ...
    archive_pwd: str | None = Field(default=None, description="Password used to extract archive")
...
class SubmitResponse(BaseModel):
    ...
    uuid: str | None = Field(default=None, description="UUID of the submitted analysis")
...
class AnalysisDetails(BaseModel):
    ...
    error: str | None = Field(default=None, description="Error message if Status is false")
    errors: dict[str, str] | None = Field(default=None, description="Error message by services")
    filenames: list[str] | None = Field(default=None, description="List of analysed filename")
    malwares: list[str] | None = Field(default=None, description="List of malware names found in analysis")
    files: list[FileResult] | None = Field(
        default=None,
        description="Array of submission files (input file and extracted sub-files)",
    )
    threats: dict[str, Threat] | None = Field(
        default=None,
        description=(
            "Summary of threats found in submission. Each submission file reaching "
            "threshold score will add an entry. Entry keys are files' SHA256"
        ),
    )
...
class ProfileStatus(BaseModel):
    ...
    available_daily_quota: int = Field(
        description=(
            "Number of submissions still available within 24h. It's a sliding window, "
            "so a new slot will be released 24h after each submission"
        )
    )
    ...
    estimated_analysis_duration: int = Field(
        description=(
            "Estimation of the duration for the next submissions in milliseconds. It's "
            "based on the average time of submissions and the submission queue state. "
...
class ExportSubmissionArguments(BaseModel):
    ...
    is_full: bool = Field(default=False, description="Export full analysis or summarized")
