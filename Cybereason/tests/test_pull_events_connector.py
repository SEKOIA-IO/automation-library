...
def test_next_batch_sleep_until_next_batch(trigger, mock_cybereason_api):
    with patch("cybereason_modules.connector_pull_events.time") as mock_time:
        mock_cybereason_api.post(
            "https://fake.cybereason.net/rest/detection/inbox",
            status_code=200,
            json={"malops": [EPP_MALOP] * 16 + [EDR_MALOP] * 18},
        )
        mock_cybereason_api.post(
            "https://fake.cybereason.net/rest/detection/details",
            status_code=200,
...
@pytest.mark.skipif(
    "{'CYBEREASON_USERNAME', 'CYBEREASON_PASSWORD', 'CYBEREASON_BASE_URL'}" ".issubset(os.environ.keys()) == False"
)
def test_fetch_next_malop_integration(symphony_storage):
    module = CybereasonModule()
    trigger = CybereasonEventConnector(module=module, data_path=symphony_storage)
    # mock the log function of trigger that requires network access to the api for reporting
    trigger.log = MagicMock()
    trigger.module.configuration = {
        "base_url": os.environ["CYBEREASON_BASE_URL"],
        "username": os.environ["CYBEREASON_USERNAME"],
        "password": os.environ["CYBEREASON_PASSWORD"],
    }
    ...
    malops = trigger.fetch_malops(int(start.timestamp()) * 1000, int(end.timestamp()) * 1000)
    assert len(malops) > 0
...
def test_fetch_last_events_integration(symphony_storage):
    module = CybereasonModule()
    trigger = CybereasonEventConnector(module=module, data_path=symphony_storage)
    # mock the log function of trigger that requires network access to the api for reporting
    trigger.log = MagicMock()
    ...
    trigger.from_date = int((end - timedelta(days=15)).timestamp()) * 1000
