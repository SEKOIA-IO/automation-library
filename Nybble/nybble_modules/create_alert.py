...
class CreateAlertAction(NybbleAction):  
  ...
  def run(self, arguments: CreateAlertArguments) -> CreateAlertResults:
    ...
    self.log(  
      message=f"Sending alert to Nybble Hub. Alert ID: {arguments.alert_data['uuid']}", level="info"
      )
    ...
    nybble_alert.alert_fields = self._generateFields(
      rule_event_fields=rule_definition["event_fields"],
      event=arguments.events[0]
    )
    ...
    nybble_alert.alert_level = (1 if arguments.alert_data["urgency"]["value"] == 0 else ceil(arguments.alert_data["urgency"]["value"] / 25)) # scale 1 to 100 in sekoia -> 1 to 4 in Nybble
    nybble_alert.rule_status = "stable"
    nybble_alert.rule_name = arguments.alert_data["title"]
    nybble_alert.rule_references = str(rule_definition["references"]).split(',') # sekoia is providing comma separated references
    nybble_alert.rule_tags = self._generateTags(rule_definition["tags"])
    nybble_alert.sigma_rule_id = rule_definition["uuid"] # TODO find better (we took RULE uuid instead of uuid_instance which is coming from alert)
    ...
    if not response.ok:
        ...
        self.error(  
          f"Send to Nybble Hub Failed: with {response.status_code}"
          )
