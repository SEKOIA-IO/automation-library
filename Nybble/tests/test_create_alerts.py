...
def arguments():
    return CreateAlertArguments(
        alert_data={
            ...
            "rule": {
                "uuid": "8b4b4a25-3b15-47ea-b701-db75d7da7346"
            },
        },
        events=[
            {
                "host.name": "host1",
                "host.ip": "1.2.3.4",
                "timestamp": "2024-05-02T12:09:42.835000Z"
            },{
                "host.name": "host2",
                "host.ip": "5.6.7.8",
                "timestamp": "2024-05-02T12:09:42.835000Z"
...
SEKOIA_ALERT_RULE_MOCK: dict[str,Any] = {
    ...
    "tags": [
        {
            "uuid": "fca4002a-07c1-41e3-8efa-c2e49a171dab",
            "name": "Linux"
        }
    ],
...
def test_create_alert(symphony_storage, nybble_module, arguments):
    ...
    with requests_mock.Mocker() as mock:
        ...
        mock.post(
            f"{nybble_module.configuration.nhub_url}/conn/sekoia",
            json={"ok": "created with success"}, status_code=200
        )
...
def test_create_alert_error(symphony_storage, nybble_module, arguments):
    ...
    with requests_mock.Mocker() as mock:
        ...
        mock.post(
            f"{nybble_module.configuration.nhub_url}/conn/sekoia",
            json={"error": "dummy error"}, status_code=500
        )
        results = create_alert_action.run(arguments)
